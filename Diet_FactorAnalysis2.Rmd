...
for (i in c(2:31)){
  ...
  adjust[,i] <- ener[,i]/ener[1]
}


diet_pca <- diet
diet_pca[11:41] <-adjust
diet_pca
```
...
```{r Survery design}
diet_sur_pca <- svydesign(id = ~msk_var_psu, strata = ~msk_var_strat, weights = ~D2Weight, nest = TRUE, data = diet_pca)
...
```{r}
svymdl <- svyfactanal(~Protein+Carbs+ TotSugar + fiber + Tot_fat + Sat_fat + mfat+pfat+chol+vitA+beta_carotene+vitB1 + vitB2 + niacin + vitB6 + folic_acid + vitC + vitD + vitK + calcium + phosphorous + magnesium + iron + zinc + copper + sodium + potassium + selenium + caffeine + alcohol, diet_sur_pca, factors = 4)
...
X <- as.matrix(X)
svyloadings <- as.matrix(svyloadings)
svyloadings_ <- as.matrix(varimax(svyloadings))[[1]]

svyscores2 <- (X %*% svyloadings_)

sdp1 <- svyscores2[,1];sdp2 <- svyscores2[,2];sdp3 <- svyscores2[,3];sdp4 <- svyscores2[,4]
#svdp5 <- svyscores[,5];svdp6 <- svyscores[,6];svdp7 <- svyscores[,7]

diet$sdp1 <- as.numeric(svdp1)
diet$sdp2<- as.numeric(svdp2)
diet$sdp3 <- as.numeric(svdp3)
diet$sdp4 <- as.numeric(svdp4)
...
diet$quantileDP3<- as.factor((quantile = ntile(diet$svdp3, 4)))
diet$quantileDP1<- as.factor((quantile = ntile(diet$svdp1, 4)))
#levels(diet$quantile) <- c("greatest risk", "2nd quant", "3rd quant", "4th quant", "least risk")
levels(diet$quantileDP1) <- c("greatest risk", "2nd quant", "3rd quant", "least risk")
levels(diet$quantileDP3) <- c("greatest risk", "2nd quant", "3rd quant", "least risk")
diet$quantileDP1 <- relevel(diet$quantileDP1, ref = "least risk") 
...
fit.od.dp3 <- summary((svyglm(obj_od ~ quantileDP3 + gender + race + smoker + bmi, design = dsub2, family = "quasibinomial")))
...
```{r Age-stratified old, survey weighted factanal logistic regressions obj_od}
    ...
    fit1 <- (svyglm(obj_od ~ svdp1 + gender  + incpov_ratio + smoker + race + bmi, design = dsub1, family = "quasibinomial"))
    g1 <- glm_table_maker(fit1, 13)
    
    fit2 <- (svyglm(obj_od ~ svdp2 + gender  + incpov_ratio + smoker + race + bmi , design = dsub1, family = "quasibinomial"))
      g2 <- glm_table_maker(fit2, 13)
    
    fit3 <- (svyglm(obj_od ~ svdp3 + gender  + incpov_ratio + smoker + race + bmi, design = dsub1, family = "quasibinomial"))
    g3 <- glm_table_maker(fit3, 13)[1,]
    
    fit3 <- (svyglm(obj_od ~ svdp4 + gender  + incpov_ratio + smoker + race + bmi, design = dsub1, family = "quasibinomial"))
...
resultsOD
  ...
  fit2 <- (svyglm(obj_od ~ sdp2 + gender + Energy + incpov_ratio + smoker + race + bmi , design = dsub1, family = "quasibinomial"))
...
resultssOD = data.frame()
    ...
    fit1 <- (svyglm(sr_od ~ svdp1 + gender + incpov_ratio + smoker + race + bmi, design = dsub1, family = "quasibinomial"))
      g1 <- glm_table_maker(fit1, 13)[1,]
    
    fit2 <- (svyglm(sr_od ~ svdp2 + gender + incpov_ratio + smoker + race + bmi, design = dsub1, family = "quasibinomial"))
      g2 <- glm_table_maker(fit2, 13)[1,]
    
    fit3 <- (svyglm(sr_od ~ svdp3 + gender + incpov_ratio + smoker + race + bmi, design = dsub1, family = "quasibinomial"))
      g3 <- glm_table_maker(fit3, 13)[1,]
    
    fit4 <- (svyglm(sr_od ~ svdp4 + gender + incpov_ratio + smoker + race + bmi, design = dsub1, family = "quasibinomial"))
...
cogfit1 <- (svyglm(cerad ~ svdp3 + obj_od  + gender + incpov_ratio  + smoker + race + bmi, design = diet_sur_cog , family = "gaussian"))
